...
local Pipeline(branch, platform, event, arch='amd64', server='10.6-enterprise') = {
  ...
  upgrade(version):: {
    ...
    commands: [
      ...
      if (std.split(platform, ':')[0] == 'centos' || std.split(platform, ':')[0] == 'rockylinux') then 'docker exec -t upgrade$${DRONE_BUILD_NUMBER} bash -c "yum install -y wget procps-ng curl"',
      if (pkg_format == 'deb') then 'docker exec -t upgrade$${DRONE_BUILD_NUMBER} sed -i "s/exit 101/exit 0/g" /usr/sbin/policy-rc.d',
