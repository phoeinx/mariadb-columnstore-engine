...
local Pipeline(branch, platform, event, arch='amd64', server='10.6-enterprise') = {
  ...
  upgrade(version):: {
    ...
    commands: [
      ...
      if (pkg_format == 'deb' && !(version == "10.6.4-1" && (result == "debian11" || result == "ubuntu22.04")) && !(result == "ubuntu22.04" && (version == "10.6.5-2" || version == "10.6.7-3" || version == "10.6.8-4"))) then 'docker exec -t upgrade$${DRONE_BUILD_NUMBER}' + version + ' bash -c "./upgrade_setup_deb.sh '+ version + ' ' + result + ' ' + arch + ' $${UPGRADE_TOKEN}"',
      if (std.split(platform, ':')[0] == 'centos' || std.split(platform, ':')[0] == 'rockylinux') then 'docker exec -t upgrade$${DRONE_BUILD_NUMBER}' + version + ' bash -c "./upgrade_setup_rpm.sh '+ version + ' ' + result + ' ' + arch  + ' $${UPGRADE_TOKEN}"',
    ],
  },
  upgradelog:: {
    name: 'upgradelog',
