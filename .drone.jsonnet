...
local Pipeline(branch, platform, event, arch='amd64', server='10.6-enterprise') = {
  ...
  upgrade(version):: {
    ...
    commands: [
      ...
      'docker exec -t upgrade$${DRONE_BUILD_NUMBER}' + version + ' bash -c "./core_dumps/upgrade_data.sh"',
      'docker exec -t upgrade$${DRONE_BUILD_NUMBER}' + version + ' bash -c "./core_dumps/upgrade_veryfy.sh"',
...
EOF"',
      ...
      if (pkg_format == 'deb') then 'docker exec -t upgrade$${DRONE_BUILD_NUMBER}' + version + ' apt update --yes',
      'docker exec -t upgrade$${DRONE_BUILD_NUMBER}' + version + ' bash -c "./core_dumps/upgrade_verify.sh"',
    ],
  },
  upgradelog:: {
    name: 'upgradelog',
