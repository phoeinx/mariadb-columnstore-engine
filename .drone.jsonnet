...
local Pipeline(branch, platform, event, arch='amd64', server='10.6-enterprise') = {
  ...
  upgrade(version):: {
    ...
    commands: [
      ...
      if ((std.split(platform, ':')[0] == 'centos' || std.split(platform, ':')[0] == 'rockylinux') && !(result == "rockylinux9" && (version == "10.6.5-2" || version == "10.6.7-3" || version == "10.6.8-4"))) then 'docker exec -t upgrade$${DRONE_BUILD_NUMBER}' + version + ' bash -c "./upgrade_setup_rpm.sh '+ version + ' ' + result + ' ' + arch  + ' $${UPGRADE_TOKEN}"',
    ],
  },
  upgradelog:: {
    name: 'upgradelog',
