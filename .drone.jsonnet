...
local Pipeline(branch, platform, event, arch='amd64', server='10.6-enterprise') = {
  ...
  upgrade(version):: {
    ...
    commands: [
      ...
      if (pkg_format == 'deb' && !(version == "10.6.4-1" && (result == "debian11" || result == "ubuntu22.04"))) then 'docker exec -t upgrade$${DRONE_BUILD_NUMBER}' + version + ' bash -c "./upgrade_setup_deb.sh '+ version + ' ' + result + ' ' + arch + ' $${UPGRADE_TOKEN}"',
